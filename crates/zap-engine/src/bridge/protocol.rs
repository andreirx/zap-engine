...
pub const HEADER_FLOATS: usize = 18;
...
impl ProtocolLayout {
    ...
    pub fn new(
        ...
        let buffer_total_floats = vector_data_offset + vector_data_floats;
...
mod tests {
    ...
    fn custom_capacities_compute_correctly() {
        let layout = ProtocolLayout::new(256, 8192, 16, 64, 64, 4096);
    ...
    fn offsets_are_contiguous() {
        let layout = ProtocolLayout::new(100, 200, 10, 20, 50, 100);
        ...
        assert_eq!(layout.buffer_total_floats, layout.vector_data_offset + layout.vector_data_floats);
    }

    #[test]
    fn header_size_is_18() {
        assert_eq!(HEADER_FLOATS, 18);
        assert_eq!(HEADER_MAX_VECTOR_VERTICES, 16);
        assert_eq!(HEADER_VECTOR_VERTEX_COUNT, 17);
    }

    #[test]
    fn vector_section_comes_after_sdf() {
        let layout = ProtocolLayout::new(512, 16384, 32, 32, 128, 8192);
        // Vector section should come immediately after SDF
        assert_eq!(layout.vector_data_offset, layout.sdf_data_offset + layout.sdf_data_floats);
    }
}
