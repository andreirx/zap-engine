...
| `App.tsx` | React toolbar/HUD; sends custom events to Rust and receives mode/score events |
...
- Save/Load uses a versioned Rust JSON level document stored in browser storage, preferring IndexedDB, falling back to localStorage, then to current-session memory if the browser shell exposes neither storage API.
