...
thread_local! {
    static RUNNER: RefCell<Option<GameRunner<FlagParade>>> = RefCell::new(None);
}

fn with_runner<R>(f: impl FnOnce(&mut GameRunner<FlagParade>) -> R) -> R {
    RUNNER.with(|cell| {
...
pub fn game_init() {
    ...
    log::info!("flag-parade: initialized");
}
