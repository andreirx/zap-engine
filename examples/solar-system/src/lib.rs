...
thread_local! {
    static RUNNER: RefCell<Option<GameRunner<SolarSystem>>> = RefCell::new(None);
}

fn with_runner<R>(f: impl FnOnce(&mut GameRunner<SolarSystem>) -> R) -> R {
    RUNNER.with(|cell| {
...
pub fn game_init() {
    ...
    log::info!("solar-system: initialized");
}
