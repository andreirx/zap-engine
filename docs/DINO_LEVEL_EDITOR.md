...
The saved document is versioned (`version: 1`) and uses simple DTO fields: tile material codes, money objects, dino start/color, finish tile, and volcano tile. Loading is intentionally tolerant: missing fields fall back to the starter level, the older `ground: bool[]` shape is accepted for ground-only saves, unknown tile/color codes become safe defaults, and out-of-range positions are clamped.
...
- Edit mode also exposes Save, Load, and Reset. Save/Load use the browser's default storage slot; Reset is guarded by a Yes/Cancel dialog and does not overwrite storage until Save is pressed.
...
- Play mode: `D`/right arrow moves forward, `Space`/`W`/up arrow jumps.
